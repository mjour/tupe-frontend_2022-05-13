...
const HeaderRight = memo(({ tabs }) => {
  const [menuOpened, setMenuOpened] = useState(false);
  const [isTabsShown, showTabs] = useState(false);
  const profile = {
    uid: '23623623',
  ...
  return (
    <div className='header-right no-select'>
      <div className='flex flex-center'>
        ...
        <ul className='flex header-user nowrap'>
          <li>
            <Link href='/members'>
              <div
        ...
        <button
          type='button'
          className='mobile-tab-trigger h-full flex flex-v-center ml-2'
          onClick={() => toggleTabs(!isTabsShown)}
        >
          <i className='material-icons'>menu</i>
        </button>
      </div>
      <div className={`mobile-tabs ${isTabsShown ? 'active' : ''}`}>
        <div className='BtnCloseMobile'>
          <button type='button' onClick={() => toggleTabs(!isTabsShown)}>
            <i className='closeBtn material-icons'>close</i>
          </button>
        </div>
        <div className='flex flex-v-center flex-space-between'>
          <div className='flex flex-v-center flex-space-around flex-grow-2'>
            <div className='profile'>
              ...
              <IosShareIcon id='shareIconmobile' />
            </div>
          </div>
        </div>
        <div className='mobile-tab flex'>
          <ul>
            <li>
              <a href='/security_settings'>Security Settings</a>
            </li>
            <li>
              <a href='/referral_bonus'>Referral Bonus</a>
            </li>
            <li>
              <a href='/address_management'>Address Management</a>
            </li>
          </ul>
        </div>
        {tabs &&
          tabs.map((tab) => (
            <div className='mobile-tab' key={tab.path}>
